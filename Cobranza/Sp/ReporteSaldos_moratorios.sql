...
CREATE OR ALTER PROCEDURE dbo.ReporteSaldos_moratorios
    @idGestor     int,
    @filtro       varchar(25),
    @idU          int,
...
BEGIN
    ...
    BEGIN
        SELECT
            ...
            ,proy.Proy2_IntMoratorio                             AS intmoratorio
            ,r.[IVAIntAccesorios]                                AS ivaintaccesorios
            ,proy.Proy2_IntMoratorio                             AS ivamoratorios
            ,r.[IVA]                                             AS iva
            ,Proy2_Seguro                                        AS seguro
            ,r.[Total]                                           AS total
        ...
        FROM dbo.RptActSdosDiaria r
        INNER JOIN [dbo].[TR_Cob_Saldos_Proy_Moras] Proy ON r.Prestamo = proy.Prestamo
        INNER JOIN dbo.AsignacionCobranza ac ON ac.Prestamo = r.Prestamo
            ...
            FROM (
                SELECT d.ID_Persona, d.Txt_Direccion,
                       RANK() OVER (PARTITION BY d.ID_Persona
                                    ORDER BY d.Fecha_Ult_Accion DESC, d.Fecha_Inicio DESC, d.Id_Direccion DESC) AS rk
        ...
        WHERE r.Clasificacion IN (@clasificacion, 'OLIVO')
    ...
    BEGIN
        ...
        INNER JOIN CatGestorCoordinador ON ac.nidGestor = CatGestorCoordinador.pk_idGestor
        LEFT JOIN [Cobranza].[dbo].[RptActSdosDiaria] r ON ac.Prestamo = ac.Prestamo
        INNER JOIN [dbo].[TR_Cob_Saldos_Proy_Moras] Proy ON r.Prestamo = proy.Prestamo
        INNER JOIN dbo.ListaPrestamos lp ON ac.Prestamo = CONVERT(numeric(18,0), lp.sNoProspecto)
        LEFT JOIN (
            SELECT s.ID_Persona, s.Txt_Direccion
        ...
        WHERE pk_idCoordinador = @idu
          ...
          AND r.Liquidar1 > 0
        ORDER BY CASE
            WHEN @filtro = 'Credito'        THEN r.[Prestamo]
