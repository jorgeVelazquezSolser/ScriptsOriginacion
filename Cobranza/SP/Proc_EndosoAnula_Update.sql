...
CREATE PROCEDURE [dbo].[Proc_EndosoAnula_Update] (
	 @Id_Endoso INT
	 ,@Id_Usuario INT
)
AS
BEGIN
	DECLARE
		 @estatus INT = 0
		 ,@Guid VARCHAR(100) = ''
		 ,@Fecha_Termino DATETIME = GETDATE()
	BEGIN TRANSACTION
	BEGIN TRY
		UPDATE dbo.Endoso SET Cod_ECV_Endoso='05',Fecha_Termino=@Fecha_Termino,Id_Usuario_Anula=@Id_Usuario WHERE Id_Endoso=@Id_Endoso

		SET @estatus = 1

		SELECT @Guid=Guid, @Fecha_Termino = Fecha_Termino
		FROM dbo.Endoso
	...
	END CATCH
	SELECT @estatus AS Estatus,@Guid AS GuidEndoso, @Fecha_Termino AS FechaTermino
END
